...
The subsequent native-runtime findings and current review-world instructions are in [OVERNIGHT-REVIEW.md](OVERNIGHT-REVIEW.md). Its tests extend the original validation below.
...
All 16 indoor/outdoor grenade variants have faster near-source discharge, an early sustained-emission plateau, and a slower expanding outer layer. Source ramps begin at 60% and reach full rate at 1% of device emission duration. Near-source velocity is 1.8 with 0.8 variation; the outer layer now uses 0.95 with 0.55 variation, drag 0.3 with 0.1 variation, and gravity coefficient -0.003. Native room inspection motivated more lateral travel and less outer-layer lift. Negative gravity coefficients are visual lift, not measured gas temperatures. The faster near layer and weaker outer-layer lift approximate a cooling plume; lift does not dynamically change with temperature. Existing grenade colours, caps, LODs, occlusion, duration and repeat behavior are preserved. The script updates wind on the original native source as the device moves, without restarting its burn. Legacy indoor assets remain available for compatibility.
...
Room fog births gradually over four seconds from a nonzero box, with randomized frames, varied lifetimes and much lower per-sprite opacity. Six clearance probes constrain the birth box and a swept center move avoids the old fixed 1.4 m pull through narrow spaces. Increased source strength changes amount rather than compressing a single larger sprite cloud. Each layer caps at 12 particles, with at most four living layers per room record, at least two seconds between layers, and 64 records. Expired layers release their budget, allowing sustained fire to replenish haze without a fixed-window reset. Unemitted weight decays and caps at four. The 48-sprite bound applies to one room record; coarse grouping can divide a large or irregular room into multiple records. Ray-based room grouping and billboard bounds cannot guarantee arbitrary building containment.
...
## Earlier validation and direct acceptance

This section records the preceding implementation pass. Current native regression evidence and the added review world are documented in [OVERNIGHT-REVIEW.md](OVERNIGHT-REVIEW.md).
